...
def get_daily_inspection_total():
    return [[DailyInspection.objects.filter(category=category[0], created__startswith="{0}-{1}-".format(year,month)).count() if category[0] else\
            DailyInspection.objects.filter(created__startswith="{0}-{1}-".format(year,month)).count()\
