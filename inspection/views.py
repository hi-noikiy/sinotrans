...
class CompareChartJSONView(LineChartColorMixin, BaseLineChartView):
    ...
    def get_data(self):
        data =  [[DailyInspection.objects.filter(category=category[0], created__month__iexact=month, created__year=year).count() for category in DailyInspection.daily_insepction_category] \
